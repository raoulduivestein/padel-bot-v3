...
Gebruik `deploy/nginx-padel-bot.conf` als uitgangspunt. Die config houdt de tool zelf achter basic auth, maar laat `/invite/...`, `/takeover/...` en `/signup/...` publiek bereikbaar zodat spelers zonder login kunnen reageren. De token in de URL is de toegangssleutel.
